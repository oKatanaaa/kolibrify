...
- `completion_length_cap` – gates reward when completion tokens exceed a cap (parameterized; best used as a multiplicative guard, with optional `min_reward` to keep a non-zero penalty factor).
...
- Purpose: hard cap on completion length; typically used multiplicatively to gate overlong generations.
- Parameters: `max_completion_tokens` (required, >0), `treat_missing_as_fail` (default true), `min_reward` (default 0.0) to keep a non-zero penalty factor when the cap is exceeded or tokens are missing.
- Behavior: uses `completion_tokens` supplied by the training loop. Reward 1.0 if token count ≤ cap; `min_reward` if over. Missing token counts reward `min_reward` unless `treat_missing_as_fail=False`.
...
- Config: use `builtin: completion_length_cap` and set `max_completion_tokens` (required, >0). Optional `treat_missing_as_fail` (default true) controls what happens if a token count isn’t provided. Optional `min_reward` (default 0.0) lets you downscale to a non-zero value instead of zeroing out when the cap is exceeded or tokens are missing.
...
python_graders:
  length_cap_guard:
    ...
    init_kwargs:
      ...
      min_reward: 0.2   # penalize to 0.2 when over length cap or tokens missing
...
- `min_reward` if above the cap (default 0.0)
- Missing `completion_tokens` → `min_reward` by default, or 1.0 if `treat_missing_as_fail=False`
...
- You can point to any grader type here (built-in, python, or HTTP), but multiplicative is best for hard gates (e.g., 0/1). The recommended built-in is `completion_length_cap`, which can zero or downscale rewards (via `min_reward`) if completions exceed a token cap.
- Example:
```yaml
datasets:
  gsm8k:
